			...
			<lxslt:component prefix="my-ext" functions="formatJson,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNames)
					{
						...
						r.push(getTranslationByName("prizeBox", translations));
						r.push('</td>');
						r.push('<td class="tablebody" colspan="2">');
						var prizeList = "A,B,C,D,E,F,G,H";
						debugFeed.push(convertedPrizeValues);
						var currentPrizeIndex = getPrizeNameIndex(prizeList, winningNums[1]);
						r.push(convertedPrizeValues[currentPrizeIndex]);
						r.push('</td>');
						r.push('</tr>');
						
						r.push('<tr>');
						r.push('<td class="tablehead">');
						r.push(getTranslationByName("bonusBox", translations));
						r.push('</td>');
						r.push('<td class="tablebody" colspan="2">');
						r.push(winningNums[0]+"x");
						r.push('</td>');
						r.push('</tr>');
						...
						for(var i = 0; i<3; i++){
							r.push('<tr>');
							for(var j = 0; j<3; j++){
								r.push('<td class="tablebody">'+outcomeNums[i*3 + j]+'</td>');
							}
							r.push('</tr>');
						}
						r.push('</table>');
